...
桌面卡片可以在编辑页点「更多 → 添加至桌面」,也可以返回 Omni 首页长按卡片后点「添加至桌面」,系统会直接添加并绑定当前卡片。也可以从桌面卡片库添加 Omni 卡片,再在系统卡片编辑里选择具体卡片。锁屏卡片添加到锁屏后会先显示「点击选择卡片」占位,点占位回到 Omni 选择同尺寸锁屏卡片。详见 [锁屏卡片](./lock-screen.md)。
