...
|---|---|---|
| 桌面 `desk` | `1*2` / `2*2` / `2*4` / `4*4` / `6*4` | 主屏幕 |
| 锁屏 `lock` | `lock_circle` / `lock_2*2` / `lock_2*4` | 锁屏 |

每种 `kind + dimension` 组合在编辑器里画布大小不同,锁屏还有额外约束(只能单色、不响应触摸事件)。新建卡片时先选 `kind + dimension`。

:::tip 关于实时活动 / 控制中心
鸿蒙系统暂未提供实时活动 / 控制中心按钮的等价能力。数据模型里 `kind: 'live'` 的位子留着,API 稳定后再接。当前版本只用 `desk` + `lock`。
:::
...
:::warning
不要指望秒级刷新。需要"近实时",目前鸿蒙端没有等价的实时活动 API,做不了。
:::
